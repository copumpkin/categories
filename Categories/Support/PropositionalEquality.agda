...
open import Relation.Binary.PropositionalEquality public using () renaming (_≡_ to _≣_; refl to ≣-refl; trans to ≣-trans; sym to ≣-sym; cong to ≣-cong; cong₂ to ≣-cong₂; subst to ≣-subst; subst₂ to ≣-subst₂; isEquivalence to ≣-isEquivalence; setoid to ≣-setoid; Extensionality to ≣-Extensionality; proof-irrelevance to ≣-irrelevance)
...
≣-extʰ : ∀ {a b} {A : Set a} {B : A → Set b} {f g : ∀ {x} → B x} → (∀ {x} → f {x} ≣ g {x}) → (λ {x} → f {x}) ≣ g
≣-extʰ pf = ≣-cong (λ f {x} → f x) (≣-ext λ x → pf {x})