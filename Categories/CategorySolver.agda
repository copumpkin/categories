...
open import Categories.Functor hiding (equiv) renaming (id to idF; _≡_ to _≡F_; ∘-resp-≡ to ∘F-resp-≡F)
...
liftCategory : ∀ {o a o′ a′} (C : Category o a) → Category (o ⊔ o′) (a ⊔ a′)
liftCategory {o} {a} {o′} {a′} C = record
  { Obj = Lift {ℓ = o′} (Category.Obj C)
  ; _⇒_ = λ s d → Lift {ℓ = a′} ((lower s) ⇒ (lower d))
  ; id = lift id
  ; compose = λ m₀ m₁ → lift (lower m₀ ∘ lower m₁)
  ; ASSOC = λ f g h → ≣-cong lift C.assoc
  ; IDENTITYˡ = λ f → ≣-cong lift C.identityˡ
  ; IDENTITYʳ = λ f → ≣-cong lift C.identityʳ
  }
  where
  module C = Category C
  open C

liftFunctor : ∀ {o₀ a₀ o′₀ a′₀ o₁ a₁ o′₁ a′₁} {C : Category o₀ a₀} {D : Category o₁ a₁}
              → Functor C D → Functor (liftCategory {o′ = o′₀} {a′₀} C) (liftCategory {o′ = o′₁} {a′₁} D)
liftFunctor {o₀} {a₀} {o′₀} {a′₀} {o₁} {a₁} {o′₁} {a′₁} {C} {D} F = record
  { F₀ = λ x → lift (F.F₀ (lower x))
  ; F₁ = λ m → lift (F.F₁ (lower m))
  ; identity = ≣-cong lift F.identity
  ; homomorphism = ≣-cong lift F.homomorphism
  }
  where
    module C = Category C
    module D = Category D
    module ↑C = Category (liftCategory {o′ = o′₀} {a′₀} C)
    module ↑D = Category (liftCategory {o′ = o′₁} {a′₁} D)
    module F = Functor F

mutual
  data ReifiedMorphismHelper {o a} {C : Category o a} : Category.Obj C → Category.Obj C → Set (suc (o ⊔ a)) where
    ∥_₁∥₀_ : ∀ {D : Category o a} → (F : Functor D C) → {s d : Category.Obj D} → (m : ReifiedMorphism {C = D} s d) → ReifiedMorphismHelper (Functor.F₀ F s) (Functor.F₀ F d)

  data ReifiedMorphism {o a} {C : Category o a} : Category.Obj C → Category.Obj C → Set (suc (o ⊔ a)) where
...
⟱ : ∀ {o a} → {C : Category o a} → {s d : Category.Obj C} → ReifiedMorphism {C = C} s d → Category._⇒_ C s d
⟱ {C = C} (∥m₂∥ ∥∘∥ ∥m₁∥) = ⟱ ∥m₂∥ ∘ ⟱ ∥m₁∥
...
record ReifiedMorphismOf {o a} {C : Category o a} {s d : Category.Obj C} (m : Category._⇒_ C s d) : Set (suc (o ⊔ a)) where
  constructor reify
  field
    reified : ReifiedMorphism {C = C} s d
    .reified≡ : Category._≡_ C (⟱ {C = C} reified) m

ReifiedMorphismSimplify : ∀ {o a} → {C : Category o a} → {s d : Category.Obj C}
...
.removeIdentities : ∀ {o a} {C : Category o a} {s d : Category.Obj C} {m : Category._⇒_ C s d}
...
.removeIdentities≡ : ∀ {o a} {C : Category o a} {s d : Category.Obj C} {m₀ m₁ : Category._⇒_ C s d}
                     → (∥m₀∥ : ReifiedMorphismOf {C = C} m₀)
                     → (∥m₁∥ : ReifiedMorphismOf {C = C} m₁)
