...
Hom[-,-] : ∀ {o ℓ e} → {C : Category o ℓ e} → Bifunctor (Category.op C) C (ISetoids ℓ e)
...
Hom[_,-] : ∀ {o ℓ e} → {C : Category o ℓ e} → Category.Obj C → Functor C (ISetoids ℓ e)
...
Hom[-,_] : ∀ {o ℓ e} → {C : Category o ℓ e} → Category.Obj C → Functor (Category.op C) (ISetoids ℓ e)
Hom[-,_] {_} {ℓ} {e} {C} B = record
  { F₀ = λ x → Hom[-,-].F₀ (x , B)
  ; F₁ = λ f → Hom[-,-].F₁ (f , id)
  ; identity = Hom[-,-].identity
