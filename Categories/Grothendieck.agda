...
Elementsᵉ : ∀ {o a o′} {C : Category o a} → Functor C (Sets o′) → EasyCategory _ _ _
Elementsᵉ {o′ = o′} {C = C} F = record 
  { Obj = Obj′
  ; _⇒_ = Hom′
  ; _≡_ = _≡′_
  ; _∘_ = _∘′_
...
Elements : ∀ {o a o′} {C : Category o a} → Functor C (Sets o′) → Category _ _
Elements F = EASY Elementsᵉ F

Dom : ∀ {o a o′} {C : Category o a} → (F : Functor C (Sets o′)) -> Functor (Elements F) C
Dom {C = C} F = record {
                  F₀ = proj₁;
                  F₁ = proj₁′;
                  identity = Equiv.refl;
...
Grothendieckᵉ : ∀ {o a o′ a′} {C : Category o a} → Functor C (Categories o′ a′) → EasyCategory _ _ _
Grothendieckᵉ {o′ = o′} {a′} {C = C} F = record 
  { Obj = Obj′
  ; _⇒_ = Hom′
  ; _≡_ = _≡′_
  ; _∘_ = _∘′_
  ...
  open Category C
  open Equiv
  module Cat = Category (Categories o′ a′)
  module CatE = EasyCategory.Equiv (Categoriesᵉ o′ a′)
  module Cong {c} where
  ...
  ∘-eq {cx} {xx} {cy} {cz} f g = ≣-relevant (≣-sym (≡⇒≣ (F₁ (f ∘ g)) (F₁ f Cat.∘ F₁ g) (CatE.reflexive (homomorphism {f = g} {g = f})) xx))
  ...
  id-eq {c} {x} = ≣-relevant (≣-sym (≡⇒≣ (F₁ id) Cat.id (CatE.reflexive (identity {c})) x))
  ...
  identityˡ′ {ca , xa} {cb , xb} {f , α} = identityˡ , (begin
   ...
   where
    ...
    F₁id[α]~α = begin 
      ...
      ▹ Functor.F₁ (F₁ id) α 
        ↓⟨ OHet.ohet⇒het (CatE.reflexive identity α) ⟩ 
      ▹ α 
        ∎
  ...
  assoc′ {ca , xa} {cb , xb} {cc , xc} {cd , xd} {f , α} {g , β} {h , γ} 
   ...
   where
    ...
    eq = ≡⇒≣ (F₁ ((h ∘ g) ∘ f)) (F₁ (h ∘ g ∘ f)) (CatE.reflexive (F-resp-≡ (assoc {f = f}))) xa
    eq0 = ∘-eq h g
    eq1 = ∘-eq (h ∘ g) f
    eq2 = ∘-eq h (g ∘ f)
    eq3 = ∘-eq g f
     ...
     where
      ...
      F₁h[β]~F₁h∘g[α] = begin
        ...
        ▹ Functor.F₁ (F₁ h) (Functor.F₁ (F₁ g) α)
          ↑⟨ OHet.ohet⇒het (CatE.reflexive homomorphism α) ⟩ 
        ▹ Functor.F₁ (F₁ (h ∘ g)) α 
  ...
  ∘-resp-≡′ {a , ax} {b , bx} {c , cx} {f = f , α} {h , η} {g , γ} {i , ι} (f≡h , α~η) (g≡i , γ~ι) = ∘-resp-≡ f≡h g≡i , Het.∘-resp-∼ α~η 
    (begin 
     ...
     ▹ Functor.F₁ (F₁ f) γ                         ↓⟨ OHet.ohet⇒het (CatE.reflexive (F-resp-≡ f≡h) γ) ⟩
  ...
  promote′ (f , g) (.f , h) (≣-refl , Het.≡⇒∼ ≣-refl ≣-refl x) = ≣-cong (_,_ f) (≣-relevant x)

Grothendieck : ∀ {o a o′ a′} {C : Category o a} → Functor C (Categories o′ a′) → Category _ _
Grothendieck {o′ = o′} {a′} {C = C} F = EASY (Grothendieckᵉ {o′ = o′} {a′} {C = C} F)

Gr = Grothendieck

DomGr : ∀ {o a o′ a′} {C : Category o a} → (F : Functor C (Categories o′ a′)) → Functor (Grothendieck F) C
DomGr {C = C} F = record {
                    F₀ = proj₁;
                    F₁ = proj₁;
...
inGrᵉ : ∀ {o a o′ a′} {C : Category o a} (F : Functor C (Categories o′ a′)) → ∀ c -> EasyFunctor (Functor.F₀ F c) (Grothendieckᵉ F)
inGrᵉ {C = C} F c = record {
                     F₀ = _,_ c;
                     F₁ = λ f → C.id , GrF.coe GrF.id-eq ≣-refl f;
                     identity = EasyCategory.Equiv.refl (Grothendieckᵉ F);
                                                                ...
                                                                (Het.∘-resp-∼ (Het.coerce-resp-∼ GrF.id-eq ≣-refl)
                                                                 (Het.trans
                                                                  (Het.trans (Het.coerce-resp-∼ GrF.id-eq ≣-refl)
                                                                   (Het.sym (GrF.OHet.ohet⇒het (CatE.reflexive F.identity _))))
                                                                  (Het.coerce-resp-∼ (GrF.∘-eq C.id C.id) ≣-refl))) }
  where
    module C = Category C
    module F = Functor F
    module GrF = Groth F
    open GrF using (module Het; module Cong; module CatE)

inGr : ∀ {o a o′ a′} {C : Category o a} (F : Functor C (Categories o′ a′)) → ∀ c -> Functor (Functor.F₀ F c) (Grothendieck F)
inGr F c = EasyFunctor.functor (inGrᵉ F c)