...
Exp I = record
  { Obj = I → C.Obj
  ; Hom = λ x y → (i : I) → C.Hom (x i) (y i)
  ; _≡_ = λ f g → (i : I) → C._≡_ (f i) (g i)
  ; _∘_ = λ f g i → C._∘_ (f i) (g i)
  ; id = λ {x} i → C.id
...
reduce′ H {I} {J} F G = record
  ...
  my-F₁ : ∀ {As Bs} → L.Hom As Bs → C.Hom (my-F₀ As) (my-F₀ Bs)
  ...
  .my-homomorphism : ∀ {As Bs Cs} {fs : L.Hom As Bs} {gs : L.Hom Bs Cs} → my-F₁ (gs ∘∘ fs) ≡ (my-F₁ gs ∘ my-F₁ fs)
