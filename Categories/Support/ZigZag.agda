...
data ZigZag {c ℓ₁ ℓ₂} (Base : Preorder c ℓ₁ ℓ₂) : (x y : Preorder.Carrier Base) (begin end : Direction) → Set (c ⊔ ℓ₂) where
  zig : ∀ {x y z e} (first : Preorder._∼_ Base x y) (rest : ZigZag Base y z ↗ e) → ZigZag Base x z ↘ e
  zag : ∀ {x y z e} (first : Preorder._∼_ Base y x) (rest : ZigZag Base y z ↘ e) → ZigZag Base x z ↗ e
  slish : ∀ {x y} (last : Preorder._∼_ Base x y) → ZigZag Base x y ↘ ↘
  slash : ∀ {x y} (last : Preorder._∼_ Base y x) → ZigZag Base x y ↗ ↗
...
data Alternating {c ℓ₁ ℓ₂} (Base : Preorder c ℓ₁ ℓ₂) (x y : Preorder.Carrier Base) : Set (c ⊔ ℓ₂) where
  disorient : ∀ {begin end} (zz : ZigZag Base x y begin end) → Alternating Base x y
