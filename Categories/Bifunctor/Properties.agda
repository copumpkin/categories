...
open import Categories.Bifunctor hiding (_∘_)
...
Curryˡ {C = C} {D} {E} F = record
  ...
  ; homomorphism = λ {X Y Z f g} → NT.promote (Curryˡ₁ (g C.∘ f)) (Curryˡ₁ g ∘₁ Curryˡ₁ f) (≣-trans (≣-cong (λ h → F₁ (g C.∘ f , h)) (≣-sym D.identityˡ)) F.homomorphism)
  }
  where
  module C = Category C
  module D = Category D
  ...
  Curryˡ₁ f = record
    { η = λ X → F₁ (f , D.id)
    ; commute = λ g → let open HomReasoning in
      begin
        F₁ (f , D.id) ∘ F₁ (C.id , g)
      ↑⟨ F.homomorphism ⟩
        F₁ (f C.∘ C.id , D.id D.∘ g)
